...
if __name__ == '__main__':
    ...
    shutil.copy(f"{cur_dir}/saddle_example.mop", f"{place}/saddle/saddle.mop")

    with open(f"{filename_start_geom[:-4]}.mop", "r", encoding="UTF-8") as p:
        par = p.readlines()
        good_par = "SADDLE " + par[0]
    ...
    with open(f"{place}/saddle/saddle_r.mop", "w", encoding="UTF-8") as good_input:
        for line in b_inp[:-1]:
        ...
        good_input.write("\n")
        good_input.write("\n")
        for line in xyz_start[2:]:
            good_input.write(line)

        good_input.write("\n")

        for line in xyz_end[2:]:
            good_input.write(line)

    #command = f"mopac {namespace.place[1:]}/saddle/saddle_r.mop "
    command = f"mopac saddle_r.mop "
    print(command)
    #command = f"/opt/anaconda/envs/NAenv/bin/mopac saddle_r.mop 2> /dev/null"
    #"/opt/anaconda/envs/NAenv/bin/mopac end.mop 2> /dev/null"
    try:
        proc = subprocess.Popen(command, shell=True, cwd=f"{namespace.place[1:]}/saddle/")
    except:
        print("mop error")
    errorcode = proc.wait()

    # -----------------------------take xyz from saddle-----------------------------------
    with open(f"{place}/saddle/saddle_r.out", "r", encoding="UTF-8") as good_saddle:
    ...
    for i,line in enumerate(saddle_xyz):
        ...
        if flag == 2:
            ...
            break
    j=saddle_xyz[start]
    print("start_xyz_saddle_reading")
    while not j.isspace():
        print(j)
        j_g = j.strip().split()[1:]
        j_good = ""
    ...
    # -----------------------------find thermo ts-----------------------------------
    shutil.copy(f"{place}/saddle/saddle_opt.xyz", f"{place}/H_thermo.xyz")

    find_first_thermo(opted_geom=f"{place}/saddle/saddle_opt.xyz", thermo_name=f"{place}/H_thermo.xyz",
                      charge=namespace.charge, mult=namespace.mult,
                      eps=namespace.eps, place=place)
    find_isotopes_thermo(opted_geom=f"{place}/saddle/saddle_opt.xyz", thermo_name=f"{place}/H_thermo.xyz",
    ...
    # -----------------------------find thermo start-----------------------------------
    shutil.copy(f"{place}/start/start_opt.xyz", f"{place}/H_thermo_start.xyz")

    find_first_thermo(opted_geom=f"{place}/start/start_opt.xyz", thermo_name=f"{place}/H_thermo_start.xyz",
                      charge=namespace.charge, mult=namespace.mult,
                      eps=namespace.eps, place=place)
    find_isotopes_thermo(opted_geom=f"{place}/saddle/saddle_opt.xyz", thermo_name=f"{place}/H_thermo_start.xyz",
                         positions=isotope_positions,
                         isotope=namespace.isotope, charge=namespace.charge,
                         mult=namespace.mult, eps=namespace.eps, place=place)
    ...
    start_H = float(df_H_column_list[1])
    ts_H = float(df_H_column_list[0])
    start_D = float(df_D_column_list[1])
    ts_D = float(df_D_column_list[0])
