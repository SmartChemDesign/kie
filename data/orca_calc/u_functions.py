...
def opt_geom_saddle(filename_inp,filename_xyz,
    ...
    #add_flag_zero(filename_inp[:-4], 18)
    ...
    #print(atoms.calc.get_zero_point_energy())
...
def find_first_thermo(opted_geom,thermo_name,
                      base_task_opt = "AUX LARGE THERMO(298,298) LET ISOTOPE BONDS DUMP=30M ITRY=15000 T=14D THREADS=30",
                      charge=0, mult="SINGLET", eps="78.4", place = "/"):
    xyz = read(opted_geom)
    ...
    #add_flag_minus(thermo_name, flag_atom, axis)
    #atoms.calc.change_isotopes_input()
    # atoms.get_total_energy()
    atoms.calc.read_results()
...
def change_isotopes_input(mopac,positions,isotope,
                          base_task_opt = "AUX LARGE THERMO(298,298) RESTART BONDS DUMP=30M ITRY=15000 T=14D THREADS=30",
                          charge=0, mult="SINGLET", eps="78.4"): # с нуля
    with open(mopac + '.mop', "r") as fd:
        ...
        if eps == 0:
            lines[2] = base_task_opt + f" CHARGE={charge} {mult}\n"
        else:
            lines[2] = base_task_opt + f" CHARGE={charge} {mult} eps={eps}\n"
...
def find_isotopes_thermo(opted_geom,thermo_name,positions,isotope,
    ...
    #dir=find_curent_directory()
    #change_isotopes_input(thermo_name,positions,isotope)
    #proc1 = subprocess.Popen(f"mopac {dir}/{thermo_name}.mop", shell=True,
...
def opt_geom(filename_inp,filename_xyz,xyz_trjry, h_num, h_neighbour, dest_num,
             base_task_opt = "BONDS AUX DUMP=30M ITRY=15000 T=14D THREADS=30",
             charge=0, mult="SINGLET", eps="78.4", place = "/", freez =[], freez_all_axis=False):
    print(filename_inp[:-4])
    xyz = read(filename_inp)
    atoms = Atoms(xyz)
    if eps ==0:
    ...
    with open(xyz_trjry,"a") as file2:
        #xyz_data2=file.readlines()
        #c=xyz_data1+xyz_data2
